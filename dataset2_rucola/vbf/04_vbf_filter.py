...
ax.set_title("Overlap Matrix: Pairwise Intersections (RuCoLA)", fontsize=13)
plt.tight_layout()
...
try:
    ...
    ax.set_title("Venn Diagram: Removed Examples (RuCoLA)", fontsize=13)
    plt.tight_layout()
...
label_display = [label_names.get(lbl, str(lbl)) for lbl in unique_labels]
...
ax.set_xticks(x)
ax.set_xticklabels(label_display, rotation=45, ha="right")
ax.set_ylabel("Count")
ax.set_title("Intersection: Removed per Class (RuCoLA)", fontsize=13)
ax.legend()
...
ax.set_xticklabels(label_display, rotation=45, ha="right")
ax.set_ylabel("Count")
ax.set_title("Union: Removed per Class (RuCoLA)", fontsize=13)
ax.legend()
...
ax.set_ylabel("Count")
ax.set_title("Vote Distribution: How Many Methods Flag Each Example (RuCoLA)", fontsize=12)
plt.tight_layout()
plt.savefig(os.path.join(RESULTS_DIR, "vote_distribution.png"), dpi=150)
plt.show()

if "error_type" in intersection_removed_df.columns:
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))

    for ax, df_rem, title in zip(
        axes,
        [intersection_removed_df, union_removed_df],
        ["Intersection", "Union"],
    ):
        et_counts = df_rem["error_type"].value_counts()
        et_counts.plot(kind="bar", ax=ax, edgecolor="black", alpha=0.7)
        ax.set_title(f"{title}: Removed by error_type (RuCoLA)", fontsize=12)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, "removal_by_error_type.png"), dpi=150)
    plt.show()
...
hybrid_stats = {
    ...
    "intersection": {
        ...
        "removed_per_class": {
            str(lbl): int((intersection_removed_df["label"] == lbl).sum())
    ...
    "union": {
        ...
        "removed_per_class": {
            str(lbl): int((union_removed_df["label"] == lbl).sum())
            for lbl in unique_labels
        },
    },
}
