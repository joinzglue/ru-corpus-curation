...
# Опционально: baseline_metrics.json, results_datamaps/datamaps_metrics.json, results_datamaps/datamaps_stats.json
...
OUTPUT_DIR  = "./checkpoints_random_dm"
...
random_metrics = {
    ...
    "method": "Random removal (DataMaps baseline)",
...
dm_metrics_path = os.path.join(RESULTS_DIR, "datamaps_metrics.json")
if os.path.exists(dm_metrics_path):
    with open(dm_metrics_path, "r", encoding="utf-8") as f:
        datamaps_metrics = json.load(f)

if baseline_metrics or datamaps_metrics:
    print(f"\n{'=' * 60}")
    print("СРАВНЕНИЕ: BASELINE vs DATAMAPS vs RANDOM")
    print(f"{'=' * 60}")

    bl_f1  = baseline_metrics.get("test_f1_macro", 0)
    bl_acc = baseline_metrics.get("test_accuracy", 0)
    dm_f1  = datamaps_metrics.get("test_f1_macro", 0)
    dm_acc = datamaps_metrics.get("test_accuracy", 0)
    rn_f1  = random_metrics["test_f1_macro"]
    rn_acc = random_metrics["test_accuracy"]

    print(f"{'Метрика':<20} {'Baseline':>10} {'DataMaps':>10} {'Random':>10}")
    print("-" * 60)
    print(f"{'F1-macro':<20} {bl_f1:>10.4f} {dm_f1:>10.4f} {rn_f1:>10.4f}")
    print(f"{'Accuracy':<20} {bl_acc:>10.4f} {dm_acc:>10.4f} {rn_acc:>10.4f}")
    print()
    print(f"{'Δ vs Baseline':<20} {'':>10} {dm_f1 - bl_f1:>+10.4f} {rn_f1 - bl_f1:>+10.4f}")
    print(f"{'Δ DataMaps vs Rand':<20} {'':>10} {'':>10} {dm_f1 - rn_f1:>+10.4f}")
    print()

    comparison = {
        "baseline": {"f1_macro": bl_f1, "accuracy": bl_acc, "train_size": baseline_metrics.get("train_size", "?")},
        "datamaps": {"f1_macro": dm_f1, "accuracy": dm_acc, "train_size": datamaps_metrics.get("train_size", "?")},
        "random_removal": {"f1_macro": rn_f1, "accuracy": rn_acc, "train_size": len(train_df), "n_removed": N_REMOVE},
        "delta": {
            "datamaps_vs_baseline": round(dm_f1 - bl_f1, 4),
            "random_vs_baseline": round(rn_f1 - bl_f1, 4),
            "datamaps_vs_random": round(dm_f1 - rn_f1, 4),
        },
    }

    comp_path = os.path.join(RESULTS_DIR, "comparison_datamaps_vs_random.json")
