# train.csv, val.csv, test.csv, config.py, ver_dedup_stats.json, all_results.json
...
id2label = {i: label_names.get(int(lbl), str(lbl)) for i, lbl in enumerate(unique_labels)}
...
target_names = [id2label[int(l)] for l in unique_labels]
...
print(f"\n{'=' * 60}")
print("СРАВНЕНИЕ: BASELINE vs DEDUP vs VER_DEDUP vs RANDOM")
print(f"{'=' * 60}")
