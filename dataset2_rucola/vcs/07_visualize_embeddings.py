...
def find_input(filename):
    ...
    raise FileNotFoundError(
        f"{filename} не найден ни в {RESULTS_DIR}/, ни в текущей папке. "
        f"Загрузи файл в Colab."
    )


LABEL_NAMES = {0: "unacceptable", 1: "acceptable"}
LABEL_COLORS = {0: "#e74c3c", 1: "#3498db"}
...
if all(isinstance(l, (int, np.integer)) for l in raw_labels):
    ...
    str_to_int = {"unacceptable": 0, "acceptable": 1}
...
ax.set_title("RuCoLA: UMAP-проекция эмбеддингов\n(окраска по классам)", fontsize=12)
...
for lbl, color in LABEL_COLORS.items():
    ...
    ax.scatter(
        coords[mask, 0], coords[mask, 1],
        c=color, s=5, alpha=0.2, edgecolors="none",
    )


def draw_pairs(pairs, color, lw=0.7, alpha=0.5):
    for (a, b, s) in pairs:
        ax.plot(
...
draw_pairs(same_target_pairs,  color="#27ae60", lw=0.8, alpha=0.6)
draw_pairs(diff_removed_pairs, color="#c0392b", lw=1.0, alpha=0.8)
draw_pairs(diff_kept_pairs,    color="#2980b9", lw=0.6, alpha=0.45)
...
ax.set_title(
    f"RuCoLA: найденные пары на UMAP-проекции\n"
