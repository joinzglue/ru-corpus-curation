...
if api_failures:
    ...
    print(f"  Заменяем compressed score 0 → {median_score} (медиана)")
...
cfg = SimpleNamespace(
    ...
    embedding_cfg=SimpleNamespace(
        shuffle=False, batch_size=DS2_EMBEDDING_BATCH_SIZE,
        save_num=800, num_workers=0,
        use_pca=False, use_mi=False,
        n_neighbors=DS2_KNN_K_DETECT,
        n_neighbors_diversity=DS2_KNN_K_DIVERSITY,
    ),
    hoc_cfg=SimpleNamespace(
        max_step=DS2_HOC_MAX_STEP, T0=None, p0=None,
        lr=DS2_HOC_LR, num_rounds=DS2_HOC_NUM_ROUNDS,
...
sns.heatmap(
    report.diagnose["T"], annot=True, fmt=".2f", cmap="YlGnBu",
    xticklabels=range(DS2_NUM_CLASSES), yticklabels=range(DS2_NUM_CLASSES), ax=ax,
)
ax.set_title("Score Transition Matrix T (RuCoLA)", fontsize=14)
