...
from .core_utils import cosDistance, cosDistance_chunked
...
def func(cfg, c_est, T_out, P_out):
    hoc_cfg = cfg.hoc_cfg
    loss = torch.tensor(0.0).to(hoc_cfg.device)
    ...
    for j in range(3):
        ...
        loss += weight[j] * torch.norm(c_est[j] - c_ana[j])
...
def get_consensus_patterns(dataset, sample, k=50, batch_size=1024):
    ...
    return all_knn_labels, all_values
