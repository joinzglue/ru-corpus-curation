...
flagged_cleanlab = get_method_indices(train_df, "label_issues.csv", text_col)
flagged_datamaps = get_method_indices(train_df, "hard_to_learn.csv", text_col)
flagged_ds2      = get_method_indices(train_df, "removed_examples.csv", text_col)
...
print(f"FAISS index: {index.ntotal} векторов, dim={dim}")

k = min(DEDUP_KNN_K, n_total)
print(f"Топ-{k} ближайших соседей...")
...
removed_per_class = {
    str(lbl): int((train_df.loc[sorted(to_remove), label_col] == lbl).sum())
    for lbl in unique_labels
} if to_remove else {str(lbl): 0 for lbl in unique_labels}
...
ax.set_title("Verified Dedup: Pair Breakdown (Dvach)", fontsize=12)
plt.tight_layout()
plt.savefig(os.path.join(RESULTS_DIR, "pair_breakdown.png"), dpi=150)
plt.show()

fig, ax = plt.subplots(figsize=(9, 5))
class_total = [int((train_df[label_col] == lbl).sum()) for lbl in unique_labels]
class_removed = [removed_per_class[str(lbl)] for lbl in unique_labels]
...
ax.set_xticks(x)
ax.set_xticklabels([str(l) for l in unique_labels], rotation=45, ha="right")
ax.set_ylabel("Count")
ax.set_title("Verified Dedup: Removed per Class (Dvach)", fontsize=13)
ax.legend()
...
if conflicting_pair_rows:
    ...
    ax.set_title("Diff-target pairs: max votes from other methods (Dvach)", fontsize=12)
    ax.legend()
    plt.tight_layout()
