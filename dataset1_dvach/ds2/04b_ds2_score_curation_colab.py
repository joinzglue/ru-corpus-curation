...
from config import (
    SEED, DS2_EMBEDDING_MODEL, DS2_NUM_CLASSES,
    DS2_KNN_K_DETECT, DS2_KNN_K_DIVERSITY,
    DS2_CONFIDENCE_PROB, DS2_HOC_MAX_STEP, DS2_HOC_LR,
    DS2_HOC_NUM_ROUNDS, DS2_DETECT_NUM_EPOCH, DS2_EMBEDDING_BATCH_SIZE,
)
...
print(f"Train: {len(train_df)}, text_col={text_col}")
...
all_embeddings = []
for i in tqdm(range(0, len(texts), DS2_EMBEDDING_BATCH_SIZE), desc="Embedding"):
    batch = texts[i:i + DS2_EMBEDDING_BATCH_SIZE]
    encoded = emb_tokenizer(batch, padding=True, truncation=True, max_length=512, return_tensors="pt").to(device)
    with torch.no_grad():
...
dataset = CustomizedDataset(feature=embeddings_np, label=compressed_scores, index=np.arange(len(compressed_scores)))

effective_sample_size = min(30000, int(len(dataset) * 0.9))
print(f"Effective sample_size: {effective_sample_size}")

cfg = SimpleNamespace(
    seed=SEED, modality="text", num_classes=DS2_NUM_CLASSES,
    feature_type="embedding", details=False, embedding_model=DS2_EMBEDDING_MODEL,
    embedding_cfg=SimpleNamespace(
        shuffle=False, batch_size=DS2_EMBEDDING_BATCH_SIZE, save_num=800,
        num_workers=0, use_pca=False, use_mi=False,
        n_neighbors=DS2_KNN_K_DETECT,
        n_neighbors_diversity=DS2_KNN_K_DIVERSITY,
    ),
    hoc_cfg=SimpleNamespace(
        max_step=DS2_HOC_MAX_STEP, T0=None, p0=None, lr=DS2_HOC_LR,
        num_rounds=DS2_HOC_NUM_ROUNDS, sample_size=effective_sample_size,
        already_2nn=False, device="cuda" if torch.cuda.is_available() else "cpu",
    ),
    detect_cfg=SimpleNamespace(
        num_epoch=DS2_DETECT_NUM_EPOCH, sample_size=effective_sample_size,
        k=DS2_KNN_K_DETECT, name="simifeat", method="rank",
    ),
    save_path=RESULTS_DIR + "/", dataset_type="ds2_dvach", device=device,
)
...
torch.save(report, os.path.join(RESULTS_DIR, "ds2_report.pt"))

fig, ax = plt.subplots(figsize=(8, 6))
sns.heatmap(report.diagnose["T"], annot=True, fmt=".2f", cmap="YlGnBu",
    xticklabels=range(DS2_NUM_CLASSES), yticklabels=range(DS2_NUM_CLASSES), ax=ax)
ax.set_title("Score Transition Matrix T (Dvach)")
plt.tight_layout()
