...
print("=" * 60)
print("CLEANLAB: ПОИСК ОШИБОК РАЗМЕТКИ")
print("=" * 60)
...
for fold_idx, (train_indices, val_indices) in folds_bar:
    ...
    fold_train_ds = Dataset.from_pandas(
        fold_train_df[[text_col, "label_id"]].rename(columns={"label_id": "label"}),
        preserve_index=False,
    )
    fold_val_ds = Dataset.from_pandas(
        fold_val_df[[text_col, "label_id"]].rename(columns={"label_id": "label"}),
        preserve_index=False,
    )
    ...
    print(f"  Средняя уверенность: {fold_probs.max(axis=1).mean():.4f}")
...
for lbl in unique_labels:
    class_issues = issues_df[issues_df[label_col] == lbl]
...
class_issue_counts = issues_df[label_col].value_counts().reindex(unique_labels, fill_value=0)
...
clean_train_df = train_df[~train_df["is_label_issue"]][[text_col, label_col]]
clean_train_df.to_csv("train_cleanlab.csv", index=False)
print(f"\nОчищенный train: {len(clean_train_df)} примеров")

issues_export = issues_df[[text_col, label_col, "predicted_label", "label_quality_score"]].copy()
...
print(f"Список ошибок: {len(issues_export)} → results_cleanlab/label_issues.csv")

top_issues = issues_export.head(20)
for i, (_, row) in enumerate(top_issues.iterrows(), 1):
    text_preview = row[text_col][:80].replace("\n", " ")
    print(f"  {i:2d}. [{row[label_col]} → {row['predicted_label']}] "
...
cl_stats = {
    ...
    "issues_per_class": {
        lbl: int(issues_df[issues_df[label_col] == lbl].shape[0])
        for lbl in unique_labels
    },
