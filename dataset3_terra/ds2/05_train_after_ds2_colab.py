# train_ds2.csv, val.csv, test.csv, config.py, baseline_metrics.json, cleanlab_metrics.json, datamaps_metrics.json
...
print(f"\n{'=' * 60}")
print("РЕЗУЛЬТАТЫ НА ТЕСТЕ (ПОСЛЕ DS2)")
print(f"{'=' * 60}")
...
metrics_path = os.path.join(RESULTS_DIR, "ds2_metrics.json")
with open(metrics_path, "w", encoding="utf-8") as f:
    json.dump(ds2_metrics, f, indent=2, ensure_ascii=False)
baseline_metrics = {}
cleanlab_metrics_loaded = {}
datamaps_metrics_loaded = {}

if os.path.exists("baseline_metrics.json"):
    with open("baseline_metrics.json", "r", encoding="utf-8") as f:
        baseline_metrics = json.load(f)

cl_path = "results_cleanlab/cleanlab_metrics.json"
if os.path.exists(cl_path):
    with open(cl_path, "r", encoding="utf-8") as f:
        cleanlab_metrics_loaded = json.load(f)

dm_path = "results_datamaps/datamaps_metrics.json"
if os.path.exists(dm_path):
    with open(dm_path, "r", encoding="utf-8") as f:
        datamaps_metrics_loaded = json.load(f)

ds_f1  = ds2_metrics["test_f1_macro"]
ds_acc = ds2_metrics["test_accuracy"]
ds_sz  = ds2_metrics["train_size"]
...
if datamaps_metrics_loaded:
    dm_f1  = datamaps_metrics_loaded.get("test_f1_macro", 0)
    dm_acc = datamaps_metrics_loaded.get("test_accuracy", 0)
    dm_sz  = datamaps_metrics_loaded.get("train_size", "?")
...
header  += f"{'DS2':>10}"
row_f1  += f"{ds_f1:>10.4f}"
row_acc += f"{ds_acc:>10.4f}"
row_sz  += f"{ds_sz:>10}"
methods["ds2"] = {"f1_macro": ds_f1, "accuracy": ds_acc, "train_size": ds_sz}
...
deltas = {}
if baseline_metrics:
    deltas["ds2_vs_baseline"] = {
        "f1_macro": round(ds_f1 - bl_f1, 4),
        "accuracy": round(ds_acc - bl_acc, 4),
    }
    print(f"DS2 vs Baseline:  F1 {ds_f1 - bl_f1:+.4f},  Acc {ds_acc - bl_acc:+.4f}")
if cleanlab_metrics_loaded:
    deltas["ds2_vs_cleanlab"] = {
        "f1_macro": round(ds_f1 - cl_f1, 4),
        "accuracy": round(ds_acc - cl_acc, 4),
    }
    print(f"DS2 vs Cleanlab:  F1 {ds_f1 - cl_f1:+.4f},  Acc {ds_acc - cl_acc:+.4f}")
if datamaps_metrics_loaded:
    deltas["ds2_vs_datamaps"] = {
        "f1_macro": round(ds_f1 - dm_f1, 4),
        "accuracy": round(ds_acc - dm_acc, 4),
    }
    print(f"DS2 vs DataMaps:  F1 {ds_f1 - dm_f1:+.4f},  Acc {ds_acc - dm_acc:+.4f}")
