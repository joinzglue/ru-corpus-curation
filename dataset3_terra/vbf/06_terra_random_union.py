# train.csv, val.csv, test.csv, config.py, hybrid_stats.json, baseline_metrics.json, union_metrics.json
...
OUTPUT_DIR  = "./checkpoints_random_union"
...
N_REMOVE = hybrid_stats["union"]["n_removed"]
...
print(f"\n{'=' * 60}")
print("РЕЗУЛЬТАТЫ НА ТЕСТЕ (РАНДОМНОЕ УДАЛЕНИЕ — UNION BASELINE)")
print(f"{'=' * 60}")
...
random_metrics = {
    ...
    "method": "Random removal (Union baseline)",
...
metrics_path = os.path.join(RESULTS_DIR, "random_union_metrics.json")
...
print(f"\n{'=' * 60}")
print("СРАВНЕНИЕ: BASELINE vs UNION vs RANDOM")
print(f"{'=' * 60}")
...
if "union" in comparison:
    u_f1 = comparison["union"]["f1_macro"]
    delta = round(u_f1 - rn_f1, 4)
    print(f"  Union vs Random:    F1 {delta:+.4f}")
    comparison["delta_union_vs_random"] = delta

if "baseline" in comparison:
    bl_f1 = comparison["baseline"]["f1_macro"]
    print(f"  Random vs Baseline: F1 {round(rn_f1 - bl_f1, 4):+.4f}")

comp_path = os.path.join(RESULTS_DIR, "comparison_random_union.json")
