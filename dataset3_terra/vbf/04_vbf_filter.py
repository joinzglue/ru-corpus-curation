...
def get_removed_indices(train, removed, key_cols):
    train_keys = train[key_cols].apply(lambda col: col.str.strip())
    removed_keys = removed[key_cols].apply(lambda col: col.str.strip())
    removed_set = set(removed_keys.itertuples(index=False, name=None))
    indices = set()
    for idx, row in train_keys.iterrows():
        if tuple(row) in removed_set:
            indices.add(idx)
    return indices
...
removed_cl = get_removed_indices(train_df, cleanlab_df, KEY_COLS)
removed_dm = get_removed_indices(train_df, datamaps_df, KEY_COLS)
removed_ds2 = get_removed_indices(train_df, ds2_df, KEY_COLS)
...
ax.set_title("Overlap Matrix: Pairwise Intersections (TERRa)", fontsize=13)
plt.tight_layout()
...
ax.set_title("Venn Diagram: Removed Examples (TERRa)", fontsize=13)
plt.tight_layout()
...
ax.set_ylabel("Count")
ax.set_title("Intersection: Removed per Class (TERRa)", fontsize=13)
ax.legend()
...
ax.set_xticklabels(unique_labels, rotation=45, ha="right")
ax.set_ylabel("Count")
ax.set_title("Union: Removed per Class (TERRa)", fontsize=13)
ax.legend()
...
ax.set_ylabel("Count")
ax.set_title("Vote Distribution: How Many Methods Flag Each Example (TERRa)", fontsize=12)
plt.tight_layout()
