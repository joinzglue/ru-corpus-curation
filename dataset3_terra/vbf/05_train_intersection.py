# train_intersection.csv, val.csv, test.csv, config.py, baseline_metrics.json, cleanlab_metrics.json,  datamaps_metrics.json, ds2_metrics.json
...
OUTPUT_DIR  = "./checkpoints_hybrid_intersection"
...
train_df = pd.read_csv("train_intersection.csv")
...
print(f"\n{'=' * 60}")
print("РЕЗУЛЬТАТЫ НА ТЕСТЕ (ПОСЛЕ INTERSECTION)")
print(f"{'=' * 60}")
...
intersection_metrics = {
    ...
    "method": "Hybrid (Intersection >= 2 methods)",
...
metrics_path = os.path.join(RESULTS_DIR, "intersection_metrics.json")
with open(metrics_path, "w", encoding="utf-8") as f:
    json.dump(intersection_metrics, f, indent=2, ensure_ascii=False)
my_f1  = intersection_metrics["test_f1_macro"]
my_acc = intersection_metrics["test_accuracy"]
my_sz  = intersection_metrics["train_size"]
...
other_methods = [
    ...
    ("Cleanlab",  "cleanlab_metrics.json"),
    ("DataMaps",  "datamaps_metrics.json"),
    ("DS2",       "ds2_metrics.json"),
]

for name, path in other_methods:
    if os.path.exists(path):
        ...
        deltas[f"intersection_vs_{name.lower()}"] = {
            "f1_macro": round(my_f1 - o_f1, 4),
            "accuracy": round(my_acc - o_acc, 4),
        }

header  += f"{'Intersect':>12}"
row_f1  += f"{my_f1:>12.4f}"
row_acc += f"{my_acc:>12.4f}"
row_sz  += f"{my_sz:>12}"
methods["intersection"] = {"f1_macro": my_f1, "accuracy": my_acc, "train_size": my_sz}
...
comp_path = os.path.join(RESULTS_DIR, "comparison_intersection.json")
