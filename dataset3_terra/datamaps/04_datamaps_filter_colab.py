...
train_ds = Dataset.from_pandas(
    train_df[["premise", "hypothesis", "label_id"]].rename(columns={"label_id": "label"}),
...
print(f"\nФильтрация по категориям (EASY_KEEP_RATIO={EASY_KEEP_RATIO}):")
...
clean_export = clean_df[["premise", "hypothesis", "label"]].copy()
clean_export.to_csv("train_datamaps.csv", index=False, encoding="utf-8-sig")

removed_export = removed_df[["premise", "hypothesis", "label", "confidence",
...
full_metrics_export = train_df[["premise", "hypothesis", "label", "confidence",
                                 "variability", "correctness", "forgetfulness",
                                 "category"]].copy()
...
fig.suptitle(f"DataMap — TERRa ({MODEL_NAME})", fontsize=16, y=1.01)
fig.tight_layout()
...
ax.set_title("Распределение Confidence по категориям — DataMaps (TERRa)")
...
if os.path.exists(cl_issues_path):
    ...
    removed_keys = set(
        zip(removed_df["premise"].str.strip(), removed_df["hypothesis"].str.strip())
    )
    cl_keys = set(
        zip(cl_issues["premise"].str.strip(), cl_issues["hypothesis"].str.strip())
    )
...
for i, (_, row) in enumerate(top_hard.iterrows(), 1):
    premise_preview = str(row["premise"])[:60].replace("\n", " ")
    hyp_preview = str(row["hypothesis"])[:40].replace("\n", " ")
    print(f"  {i:2d}. [{row['label']}] conf={row['confidence']:.4f} var={row['variability']:.4f}")
...
dm_stats = {
    ...
    "removed_per_class": {
        lbl: int((removed_df["label"] == lbl).sum())
        for lbl in unique_labels
    },
...
print(f"\n{'=' * 60}")
print("DATAMAPS ФИЛЬТРАЦИЯ ЗАВЕРШЕНА")
