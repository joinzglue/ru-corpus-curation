...
KEY_COLS = ["premise", "hypothesis"]


def get_method_indices(train, csv_path, key_cols):
    if not os.path.exists(csv_path):
        print(f"  WARN: {csv_path} не найден")
        return set()
    df = pd.read_csv(csv_path)
    train_keys = train[key_cols].apply(lambda c: c.str.strip())
    other_keys = df[key_cols].apply(lambda c: c.str.strip())
    other_set = set(other_keys.itertuples(index=False, name=None))
    indices = set()
    for idx, row in train_keys.iterrows():
        if tuple(row) in other_set:
            indices.add(idx)
    return indices


flagged_cleanlab = get_method_indices(train_df, "label_issues.csv", KEY_COLS)
flagged_datamaps = get_method_indices(train_df, "hard_to_learn.csv", KEY_COLS)
flagged_ds2 = get_method_indices(train_df, "removed_examples.csv", KEY_COLS)
print(f"  Cleanlab: {len(flagged_cleanlab)}")
print(f"  DataMaps: {len(flagged_datamaps)}")
print(f"  DS2:      {len(flagged_ds2)}")
...
print(f"Найдено пар с cos > {DEDUP_COSINE_THRESHOLD}: {len(pairs)}")
...
for (a, b, s) in pairs:
    if labels[a] == labels[b]:
        ...
        text_a = f"{premises[a]} | {hypotheses[a]}"
        text_b = f"{premises[b]} | {hypotheses[b]}"
        conflicting_pair_rows.append({
            "idx_a": a, "idx_b": b,
            "cos_sim": round(s, 4),
            "label_a": labels[a], "label_b": labels[b],
            "premise_a": premises[a], "hypothesis_a": hypotheses[a],
            "premise_b": premises[b], "hypothesis_b": hypotheses[b],
            ...
            "action": action,
            "levenshtein": Levenshtein.distance(text_a, text_b),
        })

print(f"\nРазбор пар:")
print(f"  Same target:        {same_target_pairs}  → удалили вторых")
print(f"  Diff target:        {diff_target_pairs}")
print(f"    подтверждены:     {diff_confirmed_pairs}  → удалили")
print(f"    не подтверждены:  {diff_kept_pairs}  → оставили (контрастные примеры)")
print(f"\nИтого к удалению: {len(to_remove)} ({len(to_remove) / n_total * 100:.2f}%)")
...
print(f"\ntrain_ver_dedup.csv: {len(train_clean)} примеров")

if conflicting_pair_rows:
    conflicting_df = pd.DataFrame(conflicting_pair_rows).sort_values(
        "cos_sim", ascending=False
    ...
    print(f"conflicting_pairs.csv: {len(conflicting_df)} → {conflicting_path}")
...
print(f"removed_examples.csv → {removed_csv}")

unique_labels = sorted(train_df["label"].unique())
removed_per_class = {
    int(lbl): int((train_df.loc[sorted(to_remove), "label"] == lbl).sum())
    for lbl in unique_labels
} if to_remove else {int(lbl): 0 for lbl in unique_labels}
...
ax.set_title("Verified Dedup: Pair Breakdown (TERRa)", fontsize=12)
plt.tight_layout()
...
class_removed = [removed_per_class[int(lbl)] for lbl in unique_labels]
...
ax.set_ylabel("Count")
ax.set_title("Verified Dedup: Removed per Class (TERRa)", fontsize=13)
ax.legend()
...
if conflicting_pair_rows:
    ...
    ax.set_title("Diff-target pairs: max votes from other methods (TERRa)", fontsize=12)
    ax.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, "diff_pairs_votes.png"), dpi=150)
    plt.show()
assert len(train_clean) + len(to_remove) == n_total
